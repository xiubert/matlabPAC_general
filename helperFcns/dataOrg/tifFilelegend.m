clearvars;
%script to output legend of tif / xsg files
%IMPORTANT: the contains function runs on matlab 2016 and above
datapath = uigetdir('D:\','Select folder containing tif+xsg files on day of interest');
dirinfo = dir(datapath);

% %remove directories from dirinfo:
% dirinfo = dirinfo(~cell2mat(extractfield(dirinfo,'isdir')));

%remove everything but xsg and tif files
fnames = extractfield(dirinfo,'name');
dirinfo = dirinfo(contains(fnames,["xsg","tif"]));
...
dirTif = dirinfo(contains(fnames,'tif'));
% dirTif = dirinfo(contains(fnames,['qcam','tif']));

fields = fieldnames(dirTif);
fields{1} = 'folder';
fields{2} = 'date';
fields{3} = 'bytes';
fields{4} = 'name';
dirTif = orderfields(dirTif, fields);

%sort structure by size, pulse, then gain
bytes = extractfield(dirTif,'bytes');
pulse = extractfield(dirTif,'pulse');
pulse(cellfun(@isempty,pulse)) = {'Empty'};
gain = {dirTif(1:length(dirTif)).gain};
...
folderdate = datestr(dirTif(1).date,'YYYYmmdd');
filenamenpath = [datapath '\stimfilelegend_tif_' foldername '_' folderdate '.xls'];
...
xlswrite(filenamenpath, struct2cell(dirTif)', ['A2:' Alphabet(length(fields)) num2str(length(dirTif)+1)]);
